...
const Page = async ({ params }: Props) => {
  ...
  const description =
    lang === 'uk' ? product.descriptionuk : product.descriptionru;
  const price = product.price;
  const priceUsd = product.priceUsd;
  const images = product.imgs;
  const getCondition = (featuresHtml: string): string => {
    const conditionRegexUk =
      /<td class="feature__name">Стан:<\/td>\s*<td class="feature__specification">([^<]+)</;
    const conditionRegexRu =
      /<td class="feature__name">Состояние:<\/td>\s*<td class="feature__specification">([^<]+)</;
  ...
  const keywordsToRemoveUk = [
    'Доставка:',
    'Доставка по Україні:',
    'Самовивіз:',
  ];
  const keywordsToRemoveRu = [
    'Доставка:',
    'Доставка по Украине:',
    'Самовывоз:',
  ];
  const keywordsToRemove =
    lang === 'uk' ? keywordsToRemoveUk : keywordsToRemoveRu;
