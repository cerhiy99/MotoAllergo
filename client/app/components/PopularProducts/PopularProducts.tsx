...
const PopularProducts: React.FC<PopularProductsProps> = ({
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      ...
      console.log(
        '[DEBUG] loadMoreProducts: Формований URL для API запиту:',
        apiUrl
      );
      ...
      console.log(
        `[DEBUG] loadMoreProducts: Статус відповіді API: ${res.status} ${res.statusText}`
      );

      if (!res.ok) {
        throw new Error(
          `Не вдалося отримати продукти: ${res.status} ${res.statusText}`
        );
      }
      ...
      if (
        ...
        console.warn(
          '[DEBUG] loadMoreProducts: API повернуло успішний статус, але структура даних невірна.'
        );
        setHasMore(false);
        setIsLoading(false);
      ...
      console.error(
        '[DEBUG] loadMoreProducts: Помилка під час завантаження продуктів:',
        error
      );
      setHasMore(false);
    } finally {
  ...
  if (!Array.isArray(products)) {
    ...
    return (
      <div className={styles.popularProductsWrapper}>
        ...
        <p>
          {dictionary.noProductsMessage || 'Не вдалося завантажити продукти.'}
        </p>
      </div>
    );
  }
  ...
  return (
    <div ref={sectionRef} className={styles.popularProductsWrapper}>
      ...
      {products.length === 0 ? (
        <p>
          {dictionary.noProductsMessage || 'Немає продуктів для відображення.'}
        </p>
      ) : (
        <>
          <ul className={styles.popularProductsList}>
            {products.map((product) => {
              ...
              return (
                ...
                >
                  <Link href={`/${lang}/select-goods/${product.id}`}>
                    ...
                    <p className={styles.popularProductsPara}>
                      <span>{dictionary.lotNumberLabel}</span>{' '}
                      {product.lotNumber}
                    </p>
                    <p className={styles.popularProductsPara}>
                      {product.description}
