...
const ProductContent = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const tgChatid = process.env.NEXT_PUBLIC_CHAT_ID;
    const url = `${process.env.NEXT_PUBLIC_URL}uk/select-goods/${Id}`; // заміни Id на відповідний товар
  ...
  const ImageSlider = () => {
    ...
    const imagesToShow =
      filteredImages.length > 0 ? filteredImages : [images[0]];
            ...
            <div className={styles.productSummary}>
              ...
              <div className={styles.priceWrapper}>
                <p className={styles.price}>
                  {dictionary.price.replace('{price}', price)}
                </p>
                <div className={styles.wishlist} onClick={handleWishlistClick}>
                  ...
                  >
                    <path
                      ...
                      className={
                        isInWishlist ? styles.heartActive : styles.heart
                      }
                    />
                  </svg>
                  <span>
                    {isInWishlist
                      ? dictionary.wishlist.remove
                      : dictionary.wishlist.add}
                  </span>
                </div>
              </div>
              <div className={styles.actions}>
                ...
                <div className={styles.buyButtonWrapper}>
                  ...
                  >
                    ...
                    {isInCart
                      ? dictionary.actions.inCart
                      : dictionary.actions.buy}
                  </button>
                </div>
                <button
            ...
            <div className={styles.infoSections}>
              <ExpandableSection title={dictionary.infoSections.delivery.title}>
                <ul className={styles.deliveryList}>
                  {dictionary.infoSections.delivery.items.map(
              ...
              <ExpandableSection
                title={dictionary.infoSections.warrantyAndSafety.title}
              >
                <p>{dictionary.infoSections.warrantyAndSafety.guarantee}</p>
                <ul className={styles.deliveryList}>
                  {dictionary.infoSections.warrantyAndSafety.items.map(
                    (item: string, index: number) => (
                      <li key={index}>{item}</li>
                    )
                  )}
              ...
              <ExpandableSection
                title={dictionary.infoSections.advantages.title}
                isOpenByDefault={false}
              >
                <ul className={styles.deliveryList}>
                  {dictionary.infoSections.advantages.items.map(
                    (item: string, index: number) => (
                      <li key={index}>{item}</li>
                    )
                  )}
            ...
            <div className={styles.modal}>
              ...
              <div className={styles.modalProductWrapper}>
                <div className={styles.modalProductImg}>
                  <img
                    src={`${process.env.NEXT_PUBLIC_SERVER}${images[0].src}`}
                    alt=""
                  />
                </div>
